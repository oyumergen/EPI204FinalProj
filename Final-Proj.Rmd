...
d <- d %>%
  mutate(
    BOOZE_q = as.factor(BOOZE_q),
    SEX = as.factor(SEX),
    RACE = as.factor(RACE),
    MARRY = as.factor(MARRY),
    SIZE = factor(SIZE),
    entry = (EXAM_YR * 12) + EXAM_MO,
    exit = (LAST_YR * 12) + LAST_MO + (LAST_DAY/30),
    FU = exit - entry
  )

cox_test <- coxph(Surv(FU, DEATH) ~ BOOZE_q, data = d)
summary(cox_test)

d <- d %>%
  filter(complete.cases(BOOZE_q, SEX, AGEYRS, RACE, GRADES, MARRY, BMI, AVGSMK, SIZE, FU, DEATH))


cox <- coxph(Surv(FU, DEATH) ~ BOOZE_q + SEX + RACE + GRADES + MARRY + BMI + 
